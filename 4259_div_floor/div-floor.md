...
Since `rounding` is just an `enum`, we could potentially provide aliases too, like `rounding::ceil` just being another spelling for `rounding::up`. That's basically free, it's just that in this context, `rounding::up` reads better than `rounding::ceil`.