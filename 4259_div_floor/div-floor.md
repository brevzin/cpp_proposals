...
[^rust]: Rust's `div_floor` is still unstable for signed integers, but unsigned `div_ceil` and `div_floor` are stable.
...
Additionally, I think we're already okay with the existing proposal naming its family of functions `div_*` that only return the quotient even though the existing `std::div` does not.