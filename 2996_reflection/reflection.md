...
This example has moved to [@P3617R0]{.title}.
...
This discussion moved to [@P3617R0]{.title}.
...
This facility moved to [@P3617R0]{.title}.
...
* [2.13]{.pnum} it is a `$static_assert-declaration$` ([dcl.pre]),
* [2.14]{.pnum} it is an `$attribute-declaration$` ([dcl.pre]),
* [2.15]{.pnum} it is an `$empty-declaration$` ([dcl.pre]),
...
After the example following the definition of _manifestly constant-evaluated_, introduce the new term evaluation context. [It is worded a little verbosely because the evaluation context will extend to include more things, see [@P3617R0].]{.draftnote}

::: std
::: addu

[30]{.pnum} The _evaluation context_ is a set of points within the program that determines the behavior of certain functions used for reflection ([meta.reflection]). During the evaluation of an expression `$C$` as a core constant expression, the evaluation context of an evaluation `$E$` comprises the instantiation context of `$C$` ([module.context]{.sref}).
